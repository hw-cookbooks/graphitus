...
describe 'graphitus_instance provider' do
  let(:chef_run) do
    ChefSpec::Runner.new(step_into: ['graphitus_instance']) do |node|
      node.set['graphitus']['git_repository'] = 'node_repo'
      node.set['graphitus']['git_revision'] = 'node_revision'
    end.converge('fixtures::graphitus_instance')
  end
